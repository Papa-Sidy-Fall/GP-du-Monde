import { Alimentaire, Chimique, Fragile, Incassable } from './models/Produit';
import { Maritime, Aerienne, Routiere, Coordonnee } from './models/Cargaison';
import { Colis, Client } from './models/Colis';
...
function testApplication() {
    ...
    const dakar: Coordonnee = {
        latitude: 14.6928,
        longitude: -17.4467,
        nom: "Dakar, Sénégal"
    };

    const abidjan: Coordonnee = {
        latitude: 5.3600,
        longitude: -4.0083,
        nom: "Abidjan, Côte d'Ivoire"
    };

    const paris: Coordonnee = {
        latitude: 48.8566,
        longitude: 2.3522,
        nom: "Paris, France"
    };
    ...
    const cargaisonMaritime = new Maritime("CAR-001", 5000, 1200, dakar, abidjan);
    const cargaisonAerienne = new Aerienne("CAR-002", 2000, 4200, dakar, paris);
    const cargaisonRoutiere = new Routiere("CAR-003", 8000, 800, dakar, abidjan);
    ...
    const riz = new Alimentaire("Riz parfumé", 25);
    const acide = new Chimique("Acide sulfurique", 50, 8);
    const ordinateur = new Fragile("Ordinateur portable", 2.5);
    const outils = new Incassable("Outils de menuiserie", 15);
    const fruits = new Alimentaire("Fruits tropicaux", 30);
    ...
    const acideRoutes = new Chimique("Produit chimique léger", 10, 3);
    ...
    // Créer des colis
    const colis1 = new Colis("COL-123456", expediteur, destinataire, [riz, outils]);
    const colis2 = new Colis("COL-789012", destinataire, expediteur, [ordinateur]);
    ...
    const nouvelleCargaison = new Maritime("CAR-004", 1000, 500, dakar, abidjan);
